package org.hibernate.eclipse.launch.utils;
...
public class LaunchHelper {
	...
	public static ILaunchConfiguration findLaunchConfigurationByName(String launchConfigurationTypeId, String name) throws CoreException {
		Assert.isNotNull(launchConfigurationTypeId, LaunchMessages.LaunchHelper_launch_cfg_type_cannot_be_null);
		ILaunchManager launchManager = DebugPlugin.getDefault().getLaunchManager();
	...
	public static String verifyConfigurationName(String currentName) {
		if (currentName == null || currentName.length() < 1) {
			return LaunchMessages.ConsoleConfigurationWizardPage_name_must_specified;
		}
		if (Platform.OS_WIN32.equals(Platform.getOS())) {
			...
			for (int i = 0; i < badnames.length; i++) {
				if (currentName.equals(badnames[i])) {
					return NLS.bind(LaunchMessages.ConsoleConfigurationWizardPage_bad_name, currentName);
				}
			}
		}
		...
		for (int i = 0; i < disallowedChars.length; i++) {
			...
			if (currentName.indexOf(c) > -1) {
				return NLS.bind(LaunchMessages.ConsoleConfigurationWizardPage_bad_char, c);
			}
		}

		if(existingLaunchConfiguration(currentName)) {
			return LaunchMessages.ConsoleConfigurationWizardPage_config_name_already_exist;
		}
		return null;
	}
