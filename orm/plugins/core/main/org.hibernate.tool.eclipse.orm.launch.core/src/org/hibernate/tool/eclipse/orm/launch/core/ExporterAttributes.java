...
public class ExporterAttributes
{
	...
	private List<ExporterFactory> readExporterFactories(ILaunchConfiguration configuration) throws CoreException {
	   ...
	   if(exporterNames!=null) {
		   ...
		   for (String exporterId : exporterNames) {
			   ...
			   if(expDef==null) {
				   String out = NLS.bind(Messages.ExporterAttributes_could_not_locate_exporter_for_in, extensionId, configuration.getName());
   ...
   public boolean detectManyToMany() {
	...
	public String checkExporterAttributes() {
		...
		if (notSelected) {
			msg = Messages.CodeGenerationSettingsTab_console_cfg_must_be_specified;
		}
        if (msg == null) {
    		msg = PathHelper.checkDirectory(outputPath, Messages.CodeGenerationSettingsTab_output_directory, true);
        }
        if (msg == null) {
        ...
        if (msg == null) {
            if (reverseEngineer && revengSettings.trim().length() > 0) {
                msg = PathHelper.checkFile(revengSettings, Messages.CodeGenerationSettingsTab_reveng_xml_3, true);
            }
        }
        if (msg == null) {
            if (useOwnTemplates) {
                msg = PathHelper.checkDirectory(templatePath, Messages.CodeGenerationSettingsTab_template_dir, true);
            }
        }
        if (msg == null) {
            if (reverseEngineer && !preferBasicCompositeIds) {
                msg = Messages.CodeGenerationSettingsTab_script_can_not_be_generated_correctly_without;
            }
        }
		return msg;
	}
