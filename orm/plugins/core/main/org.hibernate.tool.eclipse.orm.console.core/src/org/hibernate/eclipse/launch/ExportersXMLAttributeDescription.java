...
public class ExportersXMLAttributeDescription {
	...
	private static Document getDocument() {
		...
		if (input == null) {
			HibernateConsoleCorePlugin.getDefault().logErrorMessage("Can't read resource: " + ANT_TASKS_DESCRIPTION_PATH, (Throwable)null); //$NON-NLS-1$
			return null;
		}
		...
		try {
			...
			HibernateConsoleCorePlugin.getDefault().logErrorMessage("IOException: ", e); //$NON-NLS-1$
		} finally {
			if (in != null) {
				try {
					in.close();
	...
	public static InputStream getResInputStream(final String resName, final Class clazz) {
		...
		if (System.getSecurityManager() == null) {
			...
			input = (InputStream) AccessController.doPrivileged(new PrivilegedAction() {
				public Object run() {
