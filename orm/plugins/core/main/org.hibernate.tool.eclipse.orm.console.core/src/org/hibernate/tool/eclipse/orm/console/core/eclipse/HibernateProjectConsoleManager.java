...
public class HibernateProjectConsoleManager {
	...
	private static class ReadDatabaseMetaData extends Job {
		...
		public ReadDatabaseMetaData(IProject project, ConsoleConfiguration ccfg, TableCache cache) {
			super(Messages.HibernateNature_reading_database_metadata_for + project.getName());
			this.ccfg = ccfg;
			this.cache = cache;
		}
		...
		protected IStatus run(IProgressMonitor monitor) {
			...
			monitor.beginTask(Messages.HibernateNature_reading_database_metadata, IProgressMonitor.UNKNOWN);
			try {
				ccfg.execute(() -> {
					jcfg.readFromJDBC();
					return null;
				...
				return new Status(IStatus.ERROR, HibernateConsoleCorePlugin.ID, 1,
					Messages.HibernateNature_error_while_performing_background_reading_of_database_schema, t);
			}
		}
	}
