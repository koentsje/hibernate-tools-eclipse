	...
	public static String buildJoinTableDefaultName(Relationship relationshipReference) {
		...
		if (getJpaProject(relationshipReference).isNamingStrategyEnabled() && ns != null){
			...
			try {
				...
				Platform.getLog(NamingStrategyMappingTools.class).log(
						new Status(IStatus.ERROR, NamingStrategyMappingTools.class, m.getText(), e));
			}
		}
		return ownerTable.getName() + '_' + targetTable.getName();
	}

	public static IHibernateJpaProject getJpaProject(Relationship relationshipReference){
		return (IHibernateJpaProject)relationshipReference.getJpaProject();
	}
