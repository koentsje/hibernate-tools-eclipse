...
public class ConsoleConfigurationJavaClasspathTab extends JavaClasspathTab {
	...
	public boolean isValid(ILaunchConfiguration launchConfig) {
				...
				if (entry.getClasspathProperty() == IRuntimeClasspathEntry.USER_CLASSES 
					...
					if (entry.getType() == IRuntimeClasspathEntry.ARCHIVE) {
						if (!entry.getPath().toFile().exists()) {
							...
							String out = NLS.bind(Messages.ConsoleConfigurationTabGroup_archive_classpath_entry_does_not_exist, entry.getPath().toString());
							setErrorMessage(out);
						}
					}
				}
		...
		if (!resUserClasses) {
			setErrorMessage(Messages.ConsoleConfigurationTabGroup_classpath_must_be_set_or_restored_to_default);
		}
