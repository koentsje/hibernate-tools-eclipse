...
public class TreeToTableComposite extends Composite {
	...
	static public class MultiStateCellEditor extends CellEditor {
		...
		public MultiStateCellEditor(Composite parent, int stateCount, int initialValue) {
			super(parent);
			if(stateCount < 1) throw new IllegalArgumentException(Messages.TreeToTableComposite_incorrect_state_count);
			maxStates= stateCount;

			if(!(initialValue >= 0 && initialValue < stateCount)) throw new IllegalArgumentException(Messages.TreeToTableComposite_incorrect_initial_value);
			value= initialValue;
		...
		public static int getNextValue(int stateCount, int currentValue){
			if(!(stateCount > 1)) throw new IllegalStateException(Messages.TreeToTableComposite_incorrect_state_count);
			if(!(currentValue >= 0 && currentValue < stateCount)) throw new IllegalStateException(Messages.TreeToTableComposite_incorrect_initial_value);
		...
		protected void doSetValue(Object value) {
			...
			if(!(this.value >= 0 && this.value < maxStates)) {
				throw new IllegalStateException(Messages.TreeToTableComposite_invalid_value);
			}
		}
	}
	...
	protected String getTreeTitle() {
		return Messages.TreeToTableComposite_database_schema;
	}
	...
	private void createIncludeExcludeGroup() {
		...
		upButton.setText(Messages.TreeToTableComposite_up);
		...
		downButton.setText(Messages.TreeToTableComposite_down);
		...
		removeButton.setText(Messages.TreeToTableComposite_remove);
		...
		removeAllButton.setText(Messages.TreeToTableComposite_remove_all);
	...
	protected String[] getAddButtonLabels() {
		return new String[] { Messages.TreeToTableComposite_include, Messages.TreeToTableComposite_exclude};
	}
	...
	protected String getTableTitle() {
		return Messages.TreeToTableComposite_table_filters;
	}
	...
	private void createComposite() {
		...
		refreshButton.setText(Messages.TreeToTableComposite_refresh);
		refreshButton
