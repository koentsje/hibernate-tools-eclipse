...
public class OpenMappingAction extends SelectionListenerAction {
	...
	public OpenMappingAction() {
		super(Messages.OpenMappingAction_open_mapping_file);
		setToolTipText(Messages.OpenMappingAction_open_mapping_file);
		setEnabled(true);
	...
	public void run() {
		...
		for (int i = 0; i < paths.length; i++) {
			...
			try {
				...
				HibernateBasePlugin.getDefault().logErrorMessage(Messages.OpenMappingAction_cannot_find_mapping_file, e);
			} catch (PartInitException e) {
				HibernateBasePlugin.getDefault().logErrorMessage(Messages.OpenMappingAction_cannot_open_mapping_file, e);
	...
	public static IEditorPart run(ConsoleConfiguration consoleConfig, Object selection, Object selectionParent) throws PartInitException, JavaModelException, FileNotFoundException {
		...
		if (editorPart == null) {
   		...
   		if (editorPart == null) {
			final String title = Messages.OpenMappingAction_open_mapping_file;
			final String msg = NLS.bind(Messages.OpenMappingAction_mapping_for_not_found, selection);
	...
	public static IEditorPart run(ConsoleConfiguration consoleConfig, IProperty compositeProperty, IProperty parentProperty) 
		...
		if (file != null){
   		...
   		if (editorPart == null) {
			final String title = Messages.OpenMappingAction_open_mapping_file;
			final String msg = NLS.bind(Messages.OpenMappingAction_mapping_file_for_property_not_found, compositeProperty.getName());
			MessageDialog.openError(null, title, msg);
   			throw new FileNotFoundException(msg);
   		}
   		return editorPart;
	...
	public static boolean updateEditorSelection(IEditorPart editorPart, Object selection, IService service) {
		...
		IJavaProject proj = ProjectUtilsUI.findJavaProject(editorPart);
	...
	public static boolean updateEditorSelection(IEditorPart editorPart, IProperty compositeProperty, IProperty parentProperty, IService service) {
		...
		try {
			...
			HibernateBasePlugin.getDefault().logErrorMessage(Messages.OpenMappingAction_selection_not_found, e);
		}
		if (propRegion == null && parentProperty.isComposite()){
